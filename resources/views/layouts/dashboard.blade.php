        ...
        <nav class="flex-1 overflow-y-auto py-3 lg:py-4 px-2 lg:px-3 space-y-0.5">
            ...
            @foreach ($navItems as $item)
                ...
                <a href="{{ route($item['route']) }}"
                    ...
                    @if($isActive)
                        <span class="absolute left-0 top-1.5 bottom-1.5 w-1 bg-brand-700 rounded-r-full"></span>
                    @endif
                    <svg class="w-5 h-5 shrink-0 transition-transform duration-200 group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $item['icon'] }}" /></svg>
                    <span>{{ $item['label'] }}</span>
                </a>
            @endforeach

            <div class="pt-4 pb-1 px-3">
                <span class="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-wider">Reports & Exports</span>
            </div>

            <a href="{{ route('reports.assets-pdf') }}"
                class="relative flex items-center gap-3 px-3 lg:px-4 py-2.5 rounded-xl text-sm font-semibold text-on-surface-variant hover:bg-surface-low/80 hover:text-on-surface transition-all">
                <svg class="w-5 h-5 shrink-0 text-slate-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>
                <span>Download Assets PDF</span>
            </a>

            <a href="{{ route('reports.status-pdf') }}"
                class="relative flex items-center gap-3 px-3 lg:px-4 py-2.5 rounded-xl text-sm font-semibold text-on-surface-variant hover:bg-surface-low/80 hover:text-on-surface transition-all">
                <svg class="w-5 h-5 shrink-0 text-sky-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>
                <span>IN / OUT Status PDF</span>
            </a>
            @endforeach
        </nav>
